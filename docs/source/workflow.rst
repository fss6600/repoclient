...
Программа является клиентом, которая читает данные удаленного репозитория с пакетами, определяет наличие обновлений и производит обновление, установку и удаление подсистем.
...
После первого запуска клиента необходимо проверить и установить при отсутствии адрес репозитория, где располагаются подсистемы `ЕИИС Соцстрах`.
...
.. image:: _static/images/first_run_2.png
    :align: center
...
*Процесс обновления/установки*
    Для установки, обновления или удаления пакетов, необходимо выбрать в левой панели (*Пакеты подсистем*) необходимые пакеты,
...
.. image:: _static/images/process_start.png
    :align: center
...
**Путь к репозиторию** [*]_
    Указывает путь к репозиторию пакетов. Обязательный параметр. По-умолчанию задан FTP-сервер филиала №2. Может быть задан как ftp-серверу, сетевой или локальной папке. Например:
        ...
        `ftp://user:pass@ftp.server.name/repo/path`
